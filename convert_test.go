	...
	for _, file := range files {
		...
		t.Run(name, func(t *testing.T) {
			...
			if valuesYAML != nil && hasHelmOutput && helmErr == nil {
				cueOut := cueExport(t, cuePath, got, valuesYAML)
...
func cueExport(t *testing.T, cuePath string, cueSrc, valuesYAML []byte) []byte {
	t.Helper()

	dir := t.TempDir()

	cuePath_ := filepath.Join(dir, "output.cue")
	if err := os.WriteFile(cuePath_, cueSrc, 0o644); err != nil {
		t.Fatal(err)
	}
