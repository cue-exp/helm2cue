...
type Config struct {
	...
	// Built-in functions (printf, print, ternary) are handled by the
	// core and should not be in this map.
	Funcs map[string]PipelineFunc

	// BuiltinDefault, when set, names the template function that
	// provides a default value (e.g. "default" in Helm/Sprig).
	BuiltinDefault string

	// BuiltinRequired, when set, names the template function that
	// marks a value as required (e.g. "required" in Helm/Sprig).
	BuiltinRequired string

	// BuiltinInclude, when set, names the template function that
	// includes a named template (e.g. "include" in Helm).
	BuiltinInclude string
}
